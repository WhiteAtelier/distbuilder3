...
std::wstring
u8ToWide(const std::u8string & u8str)
{
    if (u8str.empty())
    {
        return {};
...
void
appendQuotedArgW(std::wstring & cmdline, const std::u8string & arg)
{
...
roah::distb::utils::run(const std::vector<std::u8string> & cmds)
{
    ...
    const BOOL created = CreateProcessW(nullptr, cmdline.data(), nullptr, nullptr, TRUE, 0, nullptr, nullptr, &si, &pi);
